...
description: "How ryvo models long-lived, directional payment lanes."
---

In ryvo, a payment channel is one directional payment relationship from a payer to a payee for a specific token.

In V5, channel state is stored in channel buckets. Each participant pair and token maps to a canonical lane inside a deterministic bucket.

## Why one-way

ryvo channels are intentionally unidirectional because the common case is unidirectional:
...
<Note>
  `authorized_signer` signs new cumulative payment amounts. It is the only signing key role on unilateral commitments.
</Note>
...
Older designs allowed channels to close and reopen. The current protocol removed that behavior.
...
The tradeoff is more long-lived on-chain state. The protocol accepts that tradeoff because the simpler semantics are worth it for machine-payment integrations.
