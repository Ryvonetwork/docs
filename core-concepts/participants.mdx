---
title: "Participants"
description: "How permanent, wallet-bound protocol identities work in Ryvo."
---

Every wallet that uses Ryvo registers once as a participant. That registration binds one wallet to one stable `participant_id`. The mapping does not recycle, so off-chain systems can treat `wallet -> participant_id` as stable for the life of the deployment.

## What a participant stores

The participant account is Ryvo's long-lived identity record. Each account holds:
...
The protocol does not let participants close and recreate their accounts. This preserves three simple invariants:
