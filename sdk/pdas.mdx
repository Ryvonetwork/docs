...
| --- | --- | --- |
| `AGON_PROTOCOL_PROGRAM_ID` | `PublicKey` from the packaged IDL | Matches the [live devnet deployment](/reference/deployment) by default. |
| `AGON_PROTOCOL_IDL` | The generated Anchor IDL JSON | Useful if you want to build your own `Program` from scratch. |

### Chain IDs

`AGON_CHAIN_IDS` is a `const` map used everywhere a `chainId` is required, including [`deriveMessageDomain`](#derivemessagedomain).
...
All PDA helpers return a `PublicKey` and are available both as free functions and as methods on [`AgonClient`](/sdk/client#address-helpers) (which simply curries the `programId`).
...
Channel IDs are derived from **participant IDs**, not wallet keys. Fetch a participant via [`AgonClient.fetchParticipant`](/sdk/client#account-readers) or `participantId()` to obtain one.
