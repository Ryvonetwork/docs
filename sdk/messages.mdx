...
Agon settlement consumes two signed message types, `agon-cmt-v5` (unilateral commitment) and cooperative round messages, and verifies them on-chain. The SDK gives you:
...
The block layout and `payeeRef` semantics match [Reference -> Messages -> `agon-round`](/reference/messages#agon-round).

## Ed25519 helpers

Agon's on-chain verification reads the Ed25519 native program's instruction data directly. The SDK ships four builders covering the common shapes.
...
Seven data bits per byte, continuation bit in the MSB. Matches the encoding used inside `agon-cmt-v5` and clearing-round messages. Returns a `number[]`.
...
This is the minimum viable settlement transaction in `@agonx402/sdk`. See [Recipes](/sdk/recipes) for the full lifecycle including participant registration, channel creation, and withdrawals.
