...
Anchor raises its own error types for RPC and program errors. `AgonClient` does not wrap them, let them bubble up and inspect `error.error?.errorCode` / `error.logs` as usual. The canonical list of protocol error codes is on [Reference -> Errors](/reference/errors).
