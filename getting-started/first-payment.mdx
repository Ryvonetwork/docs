...
This guide shows the smallest useful ryvo integration: one payer, one payee, one token, one payment channel, and one settled `ryvo-cmt-v5` message.
...
<Tip>
If you don't need that level of control, the official [`@ryvonetwork/sdk`](/sdk) wraps every step on this page into one or two method calls. See the SDK [Recipes](/sdk/recipes) for the same flow in a dozen lines.
</Tip>
