...
Cooperative clearing rounds are the densest settlement path in Agon.
...
With concrete numbers, `A -> B = 50`, `B -> C = 10`, `C -> A = 10`, Agon advances every included channel to its new cumulative target in one transaction.
...
For the exact byte layout, see [Message formats](/reference/messages#agon-round).
...
- [Message formats](/reference/messages#agon-round)
