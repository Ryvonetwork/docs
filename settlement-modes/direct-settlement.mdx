...
Direct settlement is the simplest path in Agon V4.

One payer signs one `agon-cmt-v5` message for one channel. The payee, or a delegated `authorized_settler` named in the message, submits it with `settle_individual`.
