...
2. `agon-round` - cooperative clearing-round message body used by BLS-authenticated clearing rounds.
...
- Use `agon-round` when several participants are willing to sign one shared message so many channels can be advanced in one on-chain transaction.
