...
description: "Integrate the ryvo Gateway into your own client or service."
---
...
```ts
const catalog = await fetch("https://gateway.ryvo.network/v1/catalog").then((r) => r.json());
...
async function fetchBalance(account: string) {
  const res = await gatewayFetch(
    "https://gateway.ryvo.network/v1/x402/solana/mainnet/helius/rpc/getBalance",
    {
      method: "POST",
      headers: { "content-type": "application/json" },
...
async function searchAsset(q: string) {
  const url = new URL("https://gateway.ryvo.network/v1/x402/tokens/assets/search");
